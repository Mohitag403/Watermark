...
async def download_link(links, message, count, raw_text2, res, raw_text0, raw_text3, thumb):
    for i in range(count - 1, len(links)):
        ...
        elif 'videos.classplusapp' in url or 'awebvideos.classplusapp' in url:
        ...
        name = f'{str(count).zfill(3)}) {message.chat.id}{name1[:60]}'
...
async def account_login(_, message):
    ...
    try:
        thread = threading.Thread(target=lambda: asyncio.run(download_link(links, message, count, raw_text2, res, raw_text0, raw_text3, thumb)))
        thread.start()
