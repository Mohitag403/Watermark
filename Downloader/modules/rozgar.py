...
async def rozgar_login(_, message):
    global cancel
    cancel = False
    editable = await message.reply_text(
        "Send **ID & Password** in this manner, otherwise, the bot will not respond.\n\nSend like this: **ID*Password**"
    )
    raw_url = "https://rozgarapinew.teachx.in/post/userLogin"
    hdr = {
        "Auth-Key": "appxapi",
        "User-Id": "-2",
        "Authorization": "",
    ...
    try:
        ...
        for y in range(0,len(xv)):
            ...
            res4 = requests.get(f"https://rozgarapinew.teachx.in/get/livecourseclassbycoursesubtopconceptapiv3?topicid=" + t + "&start=-1&courseid=" + raw_text2 + "&subjectid=" + raw_text3,headers=hdr11).json()
